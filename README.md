...
`pnpm dev` runs both. The client calls the relative `/api/v1`, and Vite proxies
it to the API — so development is same-origin, and there is no CORS anywhere in
this project. If the API runs on another port, set `VITE_API_TARGET` in
`web/.env` rather than editing `vite.config.ts`.

In production the API serves the built client itself (`SpaAdapter`), so it is
one process and one deploy, same-origin again. `pnpm build` builds the server
first, then the client — that order is required, because the client's types
come from the server's `kick typegen` output.

### How the client stays in sync with the API

`web/src/lib/api.ts` is `createClient<KickApi>`, where `KickApi` is generated by
`kick typegen` from the real controllers. A renamed route or a changed request
body becomes a **compile error in the client**, not a runtime 404. There are no
