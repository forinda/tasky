...
Everything at the repo root except `node_modules/`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `docs/`, and `plan.md` moves into `server/`. The root then gains a new `package.json`, `.gitignore`, `.gitattributes`, `README.md`, and a short workspace-level `CLAUDE.md`. It does **not** get a copy of `.agents/` — `server/.agents/` is the single canonical copy, regenerated by `kick g agents -f`.
...
- Create: a short workspace-level `CLAUDE.md` at the root (NOT a copy of `server/`'s — a ~20-line pointer). No root `.agents/`; `server/.agents/` stays the only copy.
...
- [ ] `ls -a` at the root shows only `.git`, `.gitattributes`, `.gitignore`, `CLAUDE.md`, `README.md`, `docs`, `node_modules`, `package.json`, `plan.md`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `server`, `.vscode`, `.superpowers`. There is deliberately **no** root `.agents/` — `server/.agents/` is the only copy, since `kick g agents -f` regenerates that one and nothing regenerates a root duplicate.
